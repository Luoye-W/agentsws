...
| # | 官方的做法 | 我们现在 | 判断 | WP95 | 理由 |
|---|---|---|---|---|---|
| 1 | **布局 kit 与产品分层**（dockkit 不知道 dsh） | `rail-*` 五个组件里布局与产品文案是混在一起的 | **不借** | **未做**（不借） | 我们一次只开一个面板、不分屏、不浮动——分屏树那套引擎对"一次一个"是纯负担。分层的收益要等真出现分屏需求 |
| 2 | **两段式 slot 注册**（类型声明 + 身体组件，各带 disposer） | 面板是写死在图标轨数组里的 | **借形** | **已做** `rail/registry.ts` | 这是"第三方面板"与"面板生命周期"的前提。我们迟早要让应用包（docs/23）往第三栏塞面板，那时没有注册表就只能改核心代码 |
| 3 | **`priority` 三档 + pattern 长度 + 注册顺序**决定谁开某个资源，`canOpen` 一票否决 | 没有"按资源地址开面板"这回事 | **待定** | **已做** `resolvePanel()` | 要等 #7（证据面板按对象地址开）真做了才有意义。先记着这套排序规则，别自己发明一套 |
| 4 | **内置面板走与第三方一模一样的公开路** | — | **借形** | **已做** `rail/builtin-panels.tsx` | 纪律，不是功能。一旦有了 #2，这条要一起立：官方自己不走后门，我们也不许 |
| 5 | **布局持久化：存结构不存内容**，刷新后各面板拿 tab 身份 + 资源地址自己找回来 | 只存"开没开 + 宽度" | **借形** | **已做** `agentsws.rightrail.layout` | 我们的面板只要多一个"打开哪个对象"（哪条记忆、哪个事项的证据）就立刻需要这条。**"不存内容"这一句是重点**——存了内容就等于在 localStorage 里放业务数据，和 31 §3「数据留本地但有主」对不上 |
| 6 | **启动时只读布局的 key，不挂载休眠内容、不激活 Agent** | — | **借形** | **已做** 身体一律 `lazy()` | 同上，而且这条更硬：恢复布局**不许**顺手把上次那个 Run 又跑起来 |
| 7 | **操作可重放**（铸号计数器进状态、每个动作记一条、纯函数算布局） | — | **不借** | **未做**（不借） | 这是给撤销 / 重做与分屏用的。一次只开一个面板没有可撤销的布局操作 |
| 8 | **面板绑 session**：切到全局页隐藏但不删这个会话的 tab 状态 | 面板绑**岗位 / 职责**（`rail-scope.ts`） | **不借**（形不一样，但那条"切走不删状态"要留） | **已做**（那条语义）按岗位 / 事项 / 职责分桶 | docs/54 定了岗位是任务主入口。我们该照着做的是**那条语义**：从岗位 A 切到 B 再切回来，A 的面板状态还在 |
| 9 | **收起时零代价**（没有轨、没有宽度） | 我们收起是 44px 图标轨（一直在） | **不借** | **未做**（有意不同） | 官方是"藏起来，用对话头角上一个按钮请回来"；我们的图标轨本身就是导航——十一个面板要是没有轨，人就不知道有什么可开。这是有意的不同 |
| 10 | **两种呈现共用一棵内容树**（push / fullscreen 切换不重挂） | 只有 push；< 1200 变抽屉 | **待定** | **未做**（待定） | 全屏审阅（见 #11）真做了才需要。真做时这一条是硬要求——重挂等于把人正在看的 diff 滚回顶部 |

### 3.2 面板本身

| # | 官方的面板 | 我们对应的东西 | 判断 | WP95 | 理由 |
|---|---|---|---|---|---|
| 11 | **文件改动逐文件审阅**（回合末改动卡 → 侧栏逐文件对比） | 我们的 **staged change 卡**（docs/15 变更账本；WP89 的主题副本改动现在只落成 `publish_theme` 一张卡） | **借形**（形借，体不借） | **已做** `changes-panel` + `GET /v1/changes/:id/files` | "回合结束给一张卡，点进去逐文件看改了什么"这个**形**正是建站职责最缺的一块：现在人只看得见"要不要发布"，看不见"这一轮到底改了哪几个 liquid 的哪几行"。**体不借**的三条实测理由见 §4 |
| 12 | **浏览器面板**（一 tab 一会话、iframe + 应用管历史） | WP82 的 playwright-mcp / WP92 的 BrowserSkill 会话，现在**只在事件流里看得见** | **借形** | **已做**（我们自己的组件）`run-browser-panel` + `GET /v1/runs/:id/browser` | docs/36 §9 的"自动弹"第三条（连接器要人接管浏览器）本来就要这个面板。官方那条"never injects Electron or Node access into visited content"要原样立成我们的红线 |
| 13 | **Office 预览**（本地转 PDF 再渲染） | 知识库上传的 Word / Excel（docs/19 知识对象） | **借形** | **已做（纯 JS，非官方体；含上传写口、exceljs）**（WP97 + WP99） | 形借了：知识库里一份 Word / Excel / PPT，点一下在第三栏看得见。**体没借**——官方是服务端起 LibreOffice 转 PDF（`libreoffice-kit` 平台包 259 MB，WP93 已按 16 §3 最严解释 `ignoredOptionalDependencies` 掉），我们**字节原样下发、浏览器里纯 JS 渲染**：Word 用 `docx-preview`（mammoth 因为 8 处 `new Function` 被壳的 CSP 挡死而出局），表格用 SheetJS 读成数组、表由我们画，PPT 用 JSZip 自己解每页文字与图片（**基础版**，不还原版式）。不出网、不放宽 `default-src 'self'`；≥ 20 MB 或渲染超 5 秒退成“下载原件”；文件内容不进 localStorage。这是 WP95 注册表 `resolvePanel()` 的**第一个真入口**（`agentsws://file/<源 id>/<文件名>`）。**WP99 收口三件**：① 补上**上传写口**（`POST /v1/knowledge/sources/upload` + `DELETE`，六道闸：大小 / 扩展名白名单 / 文件名洗净 / magic bytes 与扩展名对得上 / 不信客户端 MIME / 纯文本反着判；进 BlobStore + 溯源事件 `knowledge.source.added`；知识库页加拖拽上传区）——`kind: 'upload'` 从此有真数据；② 表格库 SheetJS → **`exceljs@4.4.0`（MIT）**（npm 上那一份 SheetJS 永远停在带 CVE 的 0.18.5），代价是 `.xls` 不再预览（交给下载，界面写明）、`.csv` 改成自己按 RFC 4180 解；③ 壳的 CSP 在 `default-src 'self'` 之上**只开两条**：`style-src 'self' 'unsafe-inline'`（Word 排版）与 `img-src 'self' blob: data:`（内嵌图片），`script-src` 一个字没放宽。落点见 docs/36 §11 与 docs/13 §5.1 |
| 14 | **终端面板**（选一个已装的 shell、收起后命令继续跑、关 tab 才杀进程） | 无 | **不借** | **未做**（不借） | 官方这条明写"用系统用户权限跑，不受 Agent 沙箱限制"。我们整条建站职责的价值就在那个只写得了主题副本目录的笼子里（WP89）。给人一个绕开笼子的终端，等于把 WP89 做的事拆掉 |
| 15 | **Subagent 会话开成 tab** | 我们的"运行中"面板（docs/36 §9 占位） | **借形** | **部分**：三个字段的思路落在浏览器面板上；「运行中」面板本身仍是占位 | 那三个字段值得直接拿：跑没跑完、token 用量、当前回合时长。"一次性的开成只读执行记录"这条也对——跑完的 Run 不该还能追问 |
| 16 | **提交计划预览**（`/plan` 的计划开成 tab，批 / 拒之后还能从回合末成果区再打开，重开是聚焦已有 tab） | 我们的审批项（docs/14）现在在中栏 | **待定** | **未做**（待定） | "重开是聚焦已有 tab 而不是再开一个"这条细节值得记住。但审批项在中栏还是右栏是一个 IA 决定（docs/36 §9 明写"右栏里不放聊天流"），要 Luoye 定 |
| 17 | **插件管理页**（侧栏装 / 卸 / 开关插件，实时生效） | 无；我们的应用包是 docs/23 | **不借** | **未做**（不借） | 见 UPGRADE.md 第三节 §6 第 2 条：它是"以宿主用户身份跑 pnpm 装任意包，装完的代码在沿进程里跑、在工作区沙箱之外"。与 16 §3 / 31 §3 正面冲突 |
| 18 | **工作区列表按目录层级分组** | 我们左栏是品牌 / 岗位（docs/52 / §54） | **不借** | **未做**（不借） | 他们的工作区是目录，我们的是品牌。形一样、意思完全不同 |
...
## 6. WP95 之后：这 18 条各落到哪（2026-09-18）

Luoye 在 36 §11 定 **C 档**（自有轨 + 与官方同形的注册表 + 薄适配），WP95 实现。
上面两张表各多了一列「WP95」，一句话总结：

- **四条机制全做了**：#2 两段式注册表（`apps/workstation/src/components/rail/registry.ts`）、
  #4 内置面板走同一条公开路（`builtin-panels.tsx`）、#5 布局只存结构且按**岗位 / 事项 / 职责**
  分桶（`rail-layout.ts` + `lib/ui-state.ts` 的 `agentsws.rightrail.layout`）、
  #6 启动不激活（身体一律 `lazy()`）。#3 那套排序也一起做了（`resolvePanel()`），
  第一个用上 `patterns` 的是变更审阅面板。#8 的**语义**（切走不删状态）跟着分桶自然成立。
- **三个面板的形，做了两个**：#11 变更审阅（逐文件 `git diff`，`GET /v1/changes/:id/files`）、
  #12 运行中的浏览器（**我们自己的组件**，数据是事件日志的投影 `GET /v1/runs/:id/browser`；
  官方那个 iframe 面板**不接**——WP94 spike ④ 实测它与壳 `default-src 'self'` 撞死）。
  #15 只落了"三个字段"的思路（在浏览器面板上），「运行中」面板本身仍是占位。
- **不借的六条一条都没做**（#1 / #7 / #9 / #14 / #17 / #18），**待定的四条也没动**
  （#10 全屏审阅、#13 Office 预览、#16 审批项去哪一栏）——它们要么等前置，要么等 Luoye 拍板。
  **（2026-09-18 更新：#13 已定“做”并由 WP97 落地——纯 JS 渲染、非官方体；#16 已定“仍留中栏、只借重开聚焦”。）**
- **薄适配层**（`official-adapter.ts`）只定义官方 slot 的形状与映射，
  **不引任何 `@deepseek-ai/dsh-client-*`**：spike ① 实测它非 ESM、依赖声明为空，
  ③ 一版四条破坏性改动。哪天真要接一个官方形状的面板，改的只有这一个文件。
