...
| 13 | **Office 预览**（本地转 PDF 再渲染） | 知识库上传的 Word / Excel（docs/19 知识对象） | **借形** | **已做（纯 JS，非官方体）**（WP97） | 形借了：知识库里一份 Word / Excel / PPT，点一下在第三栏看得见。**体没借**——官方是服务端起 LibreOffice 转 PDF（`libreoffice-kit` 平台包 259 MB，WP93 已按 16 §3 最严解释 `ignoredOptionalDependencies` 掉），我们**字节原样下发、浏览器里纯 JS 渲染**：Word 用 `docx-preview`（mammoth 因为 8 处 `new Function` 被壳的 CSP 挡死而出局），表格用 SheetJS 读成数组、表由我们画，PPT 用 JSZip 自己解每页文字与图片（**基础版**，不还原版式）。不出网、不放宽 `default-src 'self'`；≥ 20 MB 或渲染超 5 秒退成“下载原件”；文件内容不进 localStorage。这是 WP95 注册表 `resolvePanel()` 的**第一个真入口**（`agentsws://file/<源 id>/<文件名>`）。落点见 docs/36 §11 |
