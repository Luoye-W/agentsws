...
1. ~~`Range` 的种类清单：店铺、部门、平台账号、市场（国家）——是否够；多店铺公司每店一个 range 还是店铺组~~
   ✅ **已定**（2026-09-11，见 [44](44-多品牌、多店铺与产品线的范围模型-v1.md)，WP47 实现）：
   多店铺就是多个 `store` 范围；品牌是**范围组**（`RangeGroup`，不是新种类）；
   账号内部按平台判据切用新种类 `product_line`；`account ⊃ market`（market 的 id 写成 `账号id:站点`）；
   一个岗位可以同时挂多种范围，取并集。
