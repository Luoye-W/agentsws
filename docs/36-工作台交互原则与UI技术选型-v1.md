...
**落点（WP97，2026-09-18）——#13 Office 预览**：做了，**体不借官方**。官方那一条是服务端起 LibreOffice 转 PDF（`dsh-office-to-pdf` → `libreoffice-kit`，本机平台包 259 MB，WP93 已 `ignoredOptionalDependencies` 掉）；我们反过来——**字节原样下发，浏览器里纯 JS 渲染**，不出网、不放宽壳的 `default-src 'self'`。
