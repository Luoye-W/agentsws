...
## 10. 左栏结构（09-16 Luoye 定）

- **品牌切换器与账号 profile 在左栏最下面**（和 KOLAgents / KefuAgents 一样），顶栏只留 ⌘K、模型与积分。
- **岗位在左栏可展开**（行首 › / ˅）：展开后是该岗位的职责列表，再一行"岗位的记忆 · 技能 · 知识"。点岗位进岗位页，点职责进**职责页**。
- **每个岗位、每条职责都有自己的记忆、技能、知识、额度，用户可手改**：岗位页 tab = 卡片 / 面板 / 记录 / 记忆 / 技能 / 知识 / 额度；职责页 tab = 概览 / 记忆 / 技能 / 知识 / 额度 / 记录。记忆页列这一层的条目（来源：从哪个事项 / 复盘提升、谁批的、手动加），可改可删可"提到上一层"；下面折叠着"上面继承的"几层；右侧一张"六层怎么叠"。技能页按层列（内置包 / 公司 / 部门 / 岗位 / 职责 / 个人），本层可改；知识页 = 这条职责的适用范围与事实卡；额度页 = 职责 yml 的 caps（改了走策略变更审批）。
- 设计稿：`~/Documents/Claude/Artifacts/agentsws-workstation-v1`（左栏、职责页画板）。落地 → WP71。
