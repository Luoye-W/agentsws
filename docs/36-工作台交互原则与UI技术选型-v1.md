...
- **两档时间范围到底指哪段（09-11 WP49 修正）**：**昨天** = 工作区时区的昨天 0 点到今天 0 点，对比期是前天；**近 7 天** = 6 天前 0 点到**现在**——**包含今天**（一共 7 个自然日，今天是不完整的那天），对比期是再往前 7 整天。原来「近 7 天」是"今天 0 点往前数 7 天"，今天被排在窗外，于是今天刚下的订单明明已经拉回来了、面板上还是 0（1d 真店验收撞上的）。用户嘴里的"近 7 天"本来就包含今天。迷你走势与走势图始终 7 格，跟本期对齐（昨天档最后一格是昨天，近 7 天档最后一格是今天到现在）；**横轴日期也按工作区时区写**，不按 UTC。另：**「最近订单」不受这个开关影响**——它就是按下单时间倒序的最新 20 条；「超期未发」同理（那是当下的积压，不是某段时间发生的事）。
...
**来源与许可**：路径数据一律抄自 [Simple Icons](https://simpleicons.org)
（**CC0-1.0**，本次是 v16.30.0），每条在文件里注明 slug、品牌色（Simple Icons 的
`hex`）与抄写日期。不引 `simple-icons` 这个包——为六个图标拉三千多个进依赖不划算，
抄六条 path 进来反而看得见改了没改。
