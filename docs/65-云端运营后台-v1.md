...
agentsws 是**本地优先**的：没开通云端服务的用户，他的商品、订单、客服对话、红人名单、生成的文案全都在自己的电脑或 NAS 上（docs/21 §4.1）。云上有这些：

| 云上一直有 | 开通对应服务之后才有 | 云上永远没有 |
|---|---|---|
| 账号（一个邮箱）| 红人营销增值服务的那一份租户数据（67 §3）| 支付凭据（卡号只在 Stripe 那边）|
| 组织（计费主体）| ——红人、观测、活动、合作、交付物、追踪链接、备注 | 用户的模型 key（自带 key 不经我们）|
| 工作区关联令牌（只有 sha256）| ——联系方式那一格再包一层服务密钥 | 令牌明文（只有 sha256）|
| 钱包（两类积分的 lot）| 公共红人库（跨租户共享的一层事实，见 §10）| |
| 计量事件（能力 / 数量 / 积分 / 成本，八个字段）| | |

**「云上不许有商家业务内容」这条规则已经作废**（Luoye 2026-09-19 定，见 docs/21 §4.1）：用户要跑长程任务，数据与上下文不可避免要到我们的云端。现在的口径是"默认本地优先；开通云端服务之后，相关数据按最小必要上云，传输与静态加密，可导出可删除"。

**后台仍然不做"模拟登录"**（"以他的身份看一眼"），但**理由换了**：不是"云上没有可看的东西"（那句话现在不成立了），而是**暂不做**——它是一个能看见用户全部业务数据的口子，要配的是一整套东西（逐次审批、用户侧可见的通知、时限、全程录屏级审计）。那一套没建好之前，这个按钮的风险远大于它省下的那几次客服往返。要做的时候单独立一个 WP，不是顺手加一个按钮。
...
![红人库](assets/cloud-admin/kol.png)
