...
- 做一张**重算表**写进 `docs/77-积分价目重算-v1.md`：每条能力 → 我方成本（`cost-table.json` 的公开价 × 典型用量，含云端 Workers / DO / R2 的摊销）→ 目标毛利（**Luoye 09-23 定**：数据接口与体检类 ≥ 80%；AI 这一块——token 转发与生图**合并算一档** ≥ 40%）→ 建议价（取整到 0.1 积分；单次 ≥ 1 积分的动作要二次确认，这条口径已定）→ 与 KOLAgents 同一动作的价换算成 ¥ 后对照一列（只作参考）。**联系方式揭示**单列：KOLAgents 是 12 积分（$0.12）、我们现在多少、建议多少。
