...
describe('createConnectRuntime', () => {
  it('地址由调用方给（WP31：桌面壳不再自带一份默认值）', async () => {
    ...
    expect(status.baseUrl).toBe('http://127.0.0.1:3000')
...
describe('connectUrlFrom（WP31：地址只有一个出处）', () => {
  it('读同名环境变量；没设 / 空串 → undefined，交给服务进程兜默认值', () => {
    expect(connectUrlFrom({ [CONNECT_URL_ENV]: 'http://127.0.0.1:3100' })).toBe(
      'http://127.0.0.1:3100',
    )
    expect(connectUrlFrom({ [CONNECT_URL_ENV]: '  http://x:1  ' })).toBe('http://x:1')
    expect(connectUrlFrom({})).toBeUndefined()
    expect(connectUrlFrom({ [CONNECT_URL_ENV]: '   ' })).toBeUndefined()
  })
})
