/**
 * 一条职责的规矩：这个职责是什么、动作与额度、能自动到哪一步，以及两个动作
 * （复制一份 / 提交审批）。
 *
 * WP70（54 §4）：它**不再是公司页的一个 tab**——职责一律归在岗位下面，这张卡
 * 现在长在「岗位」tab 里那个折叠层的一条职责下面。组件留着（`RolesTab` 还是
 * 一列职责 + 一张说明），只是入口换了。
 *
 * 两条纪律照 05 §0：
...
/** 额度输入框的初值：动作 id + 额度名 → 现在的数字。 */
function capsOf(role: RoleSummaryView): Record<string, string> {
...
/**
 * 一条职责的说明卡。换一条职责时用 `key={role.id}` 重挂，改了一半的名字与额度
 * 不会跟着跑到另一条上去。
 */
...
/**
 * 一列职责 + 一张说明。WP70 之后公司页不再用它（职责长在岗位折叠层里），
 * 留给"就想挨条看职责"的地方（比如只有零散职责、没有岗位的历史工作区）。
 */
