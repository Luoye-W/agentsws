...
| GET | `/v1/wallet` | `wallet:read` / `wallet:topup` | 余额：永不过期 / 有期限 / 即将过期 / 预扣中 |
| GET | `/v1/wallet/usage?group=capability\|workspace\|day&from&to` | `wallet:read` / `wallet:topup` | 用量明细（**只聚合计量事件**） |
| GET | `/v1/wallet/pricing` | `wallet:read` / `wallet:topup` | 价目表（能力 → 单位 → 积分） |
| POST | `/v1/wallet/topup` | `wallet:read` / `wallet:topup` | 建一笔充值单（这一版只做 Stripe） |
