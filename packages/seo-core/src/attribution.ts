...
import { pageKey } from './signals.js'
...
export function landingKey(landing: string, shop_host: string): string | undefined {
  ...
  return pageKey(`${u.protocol}//${u.host}${u.pathname}`)
}
...
export function pageRevenue(input: {
  ...
  for (const r of input.gsc) rowOf(pageKey(r.page), r.page).clicks += r.clicks
  let unmatched = 0
  let campaign = 0
  for (const order of input.orders) {
    ...
    const row = rowOf(key, `https://${key}`)
    row.orders += 1
    row.revenue = round2(row.revenue + order.total)
    row.currency = order.currency
  }
  const ga4 = new Map((input.ga4 ?? []).map((g) => [pageKey(g.page), g.conversion_rate]))
