...
cd "$wt"; : > "$log"; n=0
while [ ! -f "$report" ] && [ "$n" -lt "${QWEN_MAX_ROUNDS:-25}" ]; do
  n=$((n+1)); echo "=== [qwen-wp] 第 $n 轮 $(date '+%F %T') ===" >> "$log"
  qwen --approval-mode "${QWEN_APPROVAL:-auto}" -m "${QWEN_MODEL:-qwen3.8-max}" "$prompt" >> "$log" 2>&1 || true
  if tail -n 15 "$log" | grep -qiE "invalid.?api.?key|unauthorized|401|quota|insufficient|arrearage|limit reached|not authenticated"; then
    echo "=== [qwen-wp] 鉴权或额度问题，停 ===" >> "$log"; exit 3; fi
  sleep 15
done
[ -f "$report" ]
